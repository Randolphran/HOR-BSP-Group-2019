...
DevConfParam CConfigurationDlg::GetParameter()
{
   ...
   param.pointCountPerWave = m_pointsCount;
...
BOOL CConfigurationDlg::OnInitDialog()
{
   ...
   InstantAoCtrl * instAoCtrl = InstantAoCtrl::Create();
   Array<DeviceTreeNode>* sptedDevices = instAoCtrl->getSupportedDevices();
...
void CConfigurationDlg::OnCbnSelchangeComboDevice()
{
 ...
 Select other device please!"),errorCode,des);
   ...
   int channelCountMax = instantAoCtrl->getFeatures()->getChannelCountMax();
   int32 i = 0;
...
void CConfigurationDlg::OnBnClickedOk() 
{
   ...
   if (m_pointsCount <= 0)
   {
      AfxMessageBox(_T("Points for each waveform should be greater than 0."));
      return;
   }
...
void CConfigurationDlg::OnBnClickedCancel()
{
   ...
   if (!windowText.Compare(_T("Static AO")))
   {
