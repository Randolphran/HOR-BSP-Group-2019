...
#include "AsynchronousOneWaveformAODlg.h"
...
CAsynchronousOneWaveformAODlg::CAsynchronousOneWaveformAODlg(CWnd* pParent /*=NULL*/)
: CDialog(CAsynchronousOneWaveformAODlg::IDD, pParent)
{
...
CAsynchronousOneWaveformAODlg::~CAsynchronousOneWaveformAODlg()
{
}
void CAsynchronousOneWaveformAODlg::DoDataExchange(CDataExchange* pDX)
{
...
BEGIN_MESSAGE_MAP(CAsynchronousOneWaveformAODlg, CDialog)
...
// CAsynchronousOneWaveformAODlg message handlers

LRESULT CAsynchronousOneWaveformAODlg::OnConfDevice(WPARAM w,LPARAM l)
{
   int cancle = (int)w;
   if (cancle)
   {
...
BOOL CAsynchronousOneWaveformAODlg::OnInitDialog()
{
...
void CAsynchronousOneWaveformAODlg::OnStoppedEvent(void * sender, BfdAoEventArgs * args, void * userParam)
{
   CAsynchronousOneWaveformAODlg * uParam = (CAsynchronousOneWaveformAODlg *)userParam;
   uParam->GetDlgItem(IDC_Start)->EnableWindow(TRUE);
   uParam->GetDlgItem(IDC_Configure)->EnableWindow(TRUE);
}

void CAsynchronousOneWaveformAODlg::ConfigurateDevice()
{
   ErrorCode	errorCode;
   ...
   str.Format(_T("Asynchronous One Waveform AO - Run( %s )"),WCHAR_TO_TCHAR((LPCWSTR)devInfo.Description,des));
   SetWindowText(str);
   
	errorCode = m_bufferedAoCtrl->setStreaming(false);
	CheckError(errorCode);
...
void CAsynchronousOneWaveformAODlg::SetUIStateOfChannelState(PUIStateOfChannel uiStateOfChannel, BOOL state)
{
...
void CAsynchronousOneWaveformAODlg::InitUIState()
{
   int i = 0;
   int j = 0;
...
void CAsynchronousOneWaveformAODlg::ConfiguratePanel()
{
   //InitUIState();
   ...
   // Set running Dialog's window text.
}
// If you add a minimize button to your dialog, you will need the code below
//  to draw the icon.  For MFC applications using the document/view model,
//  this is automatically done for you by the framework.
void CAsynchronousOneWaveformAODlg::OnPaint()
{
   if (IsIconic())
   {
...
HCURSOR CAsynchronousOneWaveformAODlg::OnQueryDragIcon()
{
   return static_cast<HCURSOR>(m_hIcon);
}

void CAsynchronousOneWaveformAODlg::OnCheckBoxClicked(UINT nID)
{
...
void CAsynchronousOneWaveformAODlg::OnBnClickedButtonConfig()
{
   this->EnableWindow(FALSE);
   m_confDlg.ShowWindow(SW_SHOW);
}

void CAsynchronousOneWaveformAODlg::OnStart()
{
   ...
   // Create WaveformGenerator object.
	...
	// each channel only output one waveform , so buffer length is 'CHANNEL_COUNT * m_confParam.pointCountPerWave'
	int userBufferLength = CHANNEL_COUNT * m_confParam.pointCountPerWave;
	double  *waveformData = new double[userBufferLength];
	...
	m_bufferedAoCtrl->SetData(userBufferLength,waveformData);
	...
	errorCode = m_bufferedAoCtrl->Prepare();
	CheckError(errorCode);
   
    errorCode = m_bufferedAoCtrl->Start();
    CheckError(errorCode);
...
void CAsynchronousOneWaveformAODlg::CheckError(ErrorCode errorCode)
{
