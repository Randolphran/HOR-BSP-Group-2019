...
void CAsynchronousOneBufferedAIDlg::SetTriggerPointFlagLoacation(int delayCountPerChan)
{
   if (m_wfAiCtrl->getTrigger1() != NULL)
   {
      ...
      int delayCount = m_wfAiCtrl->getTrigger1()->getDelayCount();
...
{  
   ...
   Trigger * trigger = trigger = uParam->m_wfAiCtrl->getTrigger1();
   if (trigger != NULL)
   {
...
void CAsynchronousOneBufferedAIDlg::ConfigurateDevice()
{
   ...
   CString str ;
   str.Format(_T("Asynchronous One Buffered AI with Trigger Delay to Start and Delay to Stop- Run( %s )"),WCHAR_TO_TCHAR((LPCWSTR)devInfo.Description,des));
   SetWindowText(str);
   ...
   if (m_wfAiCtrl->getFeatures()->getTriggerCount() > 1)
   {
      ...
      int show = (m_wfAiCtrl->getTrigger1()->getSource() != SignalNone)? SW_SHOW : SW_HIDE;
      (GetDlgItem(IDC_STATIC_TRIGGER))->ShowWindow(show);
      (GetDlgItem(IDC_STATIC_TRIGGERPOINT))->ShowWindow(show);
   }
...
void CAsynchronousOneBufferedAIDlg::ConfigurateGraph()
{
    ...
    double shiftMax = 1000.0 * m_confParam.sectionLength / conversionRate;
