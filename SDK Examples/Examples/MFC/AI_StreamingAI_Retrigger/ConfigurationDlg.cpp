...
DevConfParam CConfigurationDlg::GetParameter()
{
   ...
   param.clockRatePerChan = m_clockRatePerChan;
	...
	param.cycles = m_cycles;
	TCHAR_TO_WCHAR(m_profilePath, param.profilePath);
	((CComboBox*)GetDlgItem(IDC_COMBO_DEVICE))->GetWindowText(param.deviceDescription);
...
void CConfigurationDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_CYCLES, m_cycles);
...
BEGIN_MESSAGE_MAP(CConfigurationDlg, CDialog)
   //{{AFX_MSG_MAP(CConfigurationDlg)
	...
	ON_EN_CHANGE(IDC_EDIT_SECTIONLENGTH,OnEnChangeEditSectionlength)
	ON_EN_CHANGE(IDC_EDIT_SECTIONCOUNT, OnEnChangeEditSectioncount)
	ON_EN_CHANGE(IDC_EDIT_CYCLES, OnEnChangeEditCycles)
...
BOOL CConfigurationDlg::OnInitDialog()
{
	...
	if (sptedDevices->getCount() == 0)
	{
		...
		if (m_comboBox_Device.GetCount() == 0)
		{
			GetDlgItem(IDOK)->EnableWindow(FALSE);
			GetDlgItem(IDCANCEL)->EnableWindow(FALSE);
			AfxMessageBox(_T("No device to support the Retrigger Demo!"));
			return FALSE;
		}
...
void CConfigurationDlg::OnCbnSelchangeComboDevice()
{
  ...
  Select other device please!"),errorCode,des);
   ...
   MathInterval	ranges;
   ValueUnit    u = Volt;
   for (i = 0; i < ValueRanges->getLength() ; i++)
   {
   ...
   m_comboBox_ChannelCount.SetCurSel(0);
   m_comboBox_ValueRange.SetCurSel(0);
	m_clockRatePerChan = 10000.0;
	m_sectionLength = 10240;
	m_cycles = 2;
...
void CConfigurationDlg::OnBnClickedCancel()
{
   ...
   if (!windowText.Compare(_T("Streaming AI with Retrigger")))
   {
...
void CConfigurationDlg::OnEnChangeEditSectionlength()
{
	// TODO:  If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
...
void CConfigurationDlg::OnEnChangeEditSectioncount()
{
	// TODO:  If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
...
void CConfigurationDlg::OnEnChangeEditCycles()
{
	// TODO:  If this is a RICHEDIT control, the control will not
	// send this notification unless you override the CDialog::OnInitDialog()
	// function and call CRichEditCtrl().SetEventMask()
