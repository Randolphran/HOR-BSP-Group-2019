...
void CConfigurationDlg::DoDataExchange(CDataExchange* pDX)
{
   ...
   DDX_Control(pDX, IDC_ClkSrc, m_cbx_clkSrc);
   DDX_Text(pDX, IDC_SectionLength, m_sectionLength);
   DDX_Text(pDX, IDC_SectionCount, m_sectionCount);

   //for trigger
   DDX_Control(pDX, IDC_TrgSrc, m_cbx_trgSrc);
   DDX_Control(pDX, IDC_TrgAct, m_cbx_trgAct);
   DDX_Control(pDX, IDC_TrgEdge, m_cbx_trgEdge);
   DDX_Text(pDX, IDC_TrgLevel, m_trgLevel);
}
...
BEGIN_MESSAGE_MAP(CConfigurationDlg, CDialog)
   ...
   // for trigger 
   ON_CBN_SELCHANGE(IDC_TrgSrc, OnCbnSelchangeTrgsrc)
...
BOOL CConfigurationDlg::OnInitDialog()
{
   ...
   Array<DeviceTreeNode>* sptedDevices = bfdEventCounterCtrl->getSupportedDevices();
...
void CConfigurationDlg::OnCbnSelchangeComboDevice()
{
   ...
   ErrorCode errorCode = bfdEventCounterCtrl->setSelectedDevice(devInfo);
 ...
 Select other device please!"),errorCode,des);
   ...
   m_channelCountMax = bfdEventCounterCtrl->getFeatures()->getChannelCountMax();
   ...
   CString str;
   CounterClockSourceIndexer* indexer = bfdEventCounterCtrl->getFeatures()->getEcSampleClockSources();
   ...
   Array<SignalDrop>* trgSrc = bfdEventCounterCtrl->getFeatures()->getTriggerSources();
   ...
   Array<TriggerAction>*trgAct = bfdEventCounterCtrl->getFeatures()->getTriggerActions();
...
void CConfigurationDlg::OnBnClickedCancel()
{
   ...
   if (!windowText.Compare(_T("Counter_BufferedEventCounter")))
   {
...
void CConfigurationDlg::OnCbnSelchangeTrgsrc()
{
   // TODO: Add your control notification handler code here
   BufferedEventCounterCtrl* bfdEventCounterCtrl = BufferedEventCounterCtrl::Create();
   // Get current selected device from device combo box
   ...
   wchar_t w_des[MAX_DEVICE_DESC_LEN];
   DeviceInformation devInfo( TCHAR_TO_WCHAR((LPCTSTR)des,  w_des) );
   bfdEventCounterCtrl->setSelectedDevice(devInfo);

   if (bfdEventCounterCtrl->getTrigger()){
      SignalDrop trgSrc = (SignalDrop)m_cbx_trgSrc.GetItemData(m_cbx_trgSrc.GetCurSel());
      
      switch (trgSrc)
      {
      case SignalNone:
         ((CWnd*)GetDlgItem(IDC_TrgAct))->EnableWindow(FALSE);
         ((CWnd*)GetDlgItem(IDC_TrgEdge))->EnableWindow(FALSE);
         ((CWnd*)GetDlgItem(IDC_TrgLevel))->EnableWindow(FALSE);
      	break;
      case SigDiPatternMatch:
      case SigDiStatusChange:
         ((CWnd*)GetDlgItem(IDC_TrgAct))->EnableWindow(TRUE);
         ((CWnd*)GetDlgItem(IDC_TrgEdge))->EnableWindow(FALSE);
         ((CWnd*)GetDlgItem(IDC_TrgLevel))->EnableWindow(FALSE);
         break;
      default:
         ((CWnd*)GetDlgItem(IDC_TrgAct))->EnableWindow(TRUE);
         ((CWnd*)GetDlgItem(IDC_TrgEdge))->EnableWindow(TRUE);
         ((CWnd*)GetDlgItem(IDC_TrgLevel))->EnableWindow(TRUE);
         break;
      }
   }
   bfdEventCounterCtrl->Dispose();
...
void CConfigurationDlg::ComponentEnabled(bool flag)
{
   if (flag){
      ...
      ((CWnd*)GetDlgItem(IDBROWSE))->EnableWindow(TRUE);

      for(int32 i = 0; i < m_channelCountMax; i++){
         ((CWnd*)GetDlgItem(IDC_CHK0 + i))->EnableWindow(FALSE);
      }

      ((CWnd*)GetDlgItem(IDC_SectionLength))->EnableWindow(FALSE);
      ((CWnd*)GetDlgItem(IDC_SectionCount))->EnableWindow(FALSE);
      ((CWnd*)GetDlgItem(IDC_ClkSrc))->EnableWindow(FALSE);

      ((CWnd*)GetDlgItem(IDC_TrgSrc))->EnableWindow(FALSE);
      OnCbnSelchangeTrgsrc();
   } else{
      ((CWnd*)GetDlgItem(IDC_EDIT_PROFILE))->EnableWindow(FALSE);
      ((CWnd*)GetDlgItem(IDBROWSE))->EnableWindow(FALSE);

      for(int32 i = 0; i < m_channelCountMax; i++){
         ((CWnd*)GetDlgItem(IDC_CHK0 + i))->EnableWindow(TRUE);
      }

      ((CWnd*)GetDlgItem(IDC_SectionLength))->EnableWindow(TRUE);
      ((CWnd*)GetDlgItem(IDC_SectionCount))->EnableWindow(TRUE);
      ((CWnd*)GetDlgItem(IDC_ClkSrc))->EnableWindow(TRUE);

      ((CWnd*)GetDlgItem(IDC_TrgSrc))->EnableWindow(TRUE);
      OnCbnSelchangeTrgsrc();
   }
}
