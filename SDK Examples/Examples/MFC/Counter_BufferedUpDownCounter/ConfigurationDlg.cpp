...
DevConfParam CConfigurationDlg::GetParameter()
{
   ...
   param.clkSrc        = (SignalDrop)m_cbx_clkSrc.GetItemData(m_cbx_clkSrc.GetCurSel());   
   param.cntType       = (CountingType)m_cbx_countingType.GetItemData(m_cbx_countingType.GetCurSel());
	TCHAR_TO_WCHAR(m_profilePath, param.profilePath);

   param.trgAct   = (TriggerAction)(m_cbx_trgAct.GetCurSel() + 1);
   param.trgEdge  = (ActiveSignal)m_cbx_trgEdge.GetItemData(m_cbx_trgEdge.GetCurSel());
   param.trgSrc   = (SignalDrop)m_cbx_trgSrc.GetItemData(m_cbx_trgSrc.GetCurSel());
...
void CConfigurationDlg::DoDataExchange(CDataExchange* pDX)
{
   ...
   DDX_Control(pDX, IDC_CBX_SampleClkSrc, m_cbx_clkSrc);
   DDX_Text(pDX, IDC_EDIT_SectionLength, m_sectionLength);
   DDX_Text(pDX, IDC_EDIT_SectionCount, m_sectionCount);
   DDX_Control(pDX, IDC_CBX_CountingType, m_cbx_countingType);
...
BOOL CConfigurationDlg::OnInitDialog()
{
   ...
   Array<DeviceTreeNode>* sptedDevices = bfdUdCounterCtrl->getSupportedDevices();
...
void CConfigurationDlg::OnCbnSelchangeComboDevice()
{
   ...
   ErrorCode errorCode = bfdUdCounterCtrl->setSelectedDevice(devInfo);
 ...
 Select other device please!"),errorCode,des);
   ...
   m_channelCountMax = bfdUdCounterCtrl->getFeatures()->getChannelCountMax();
   ...
   CString str;
   CounterClockSourceIndexer* indexer = bfdUdCounterCtrl->getFeatures()->getUdSampleClockSources();
   ...
   int cntTypeCount = bfdUdCounterCtrl->getFeatures()->getUdCountingTypes()->getCount();
   Array<CountingType> * counttingType = bfdUdCounterCtrl->getFeatures()->getUdCountingTypes();
   WCHAR strType[64] = {0};
	TCHAR tstrType[64] = {0};
   for (int k = 0; k < cntTypeCount; ++k)
   {
      ...
      m_cbx_countingType.AddString(tstrType);
      m_cbx_countingType.SetItemData(k,counttingType->getItem(k));
   }
   m_cbx_countingType.SetCurSel(0);

   //Set trigger function layout
   Array<SignalDrop>* trgSrc = bfdUdCounterCtrl->getFeatures()->getTriggerSources();
   ...
   Array<TriggerAction>*trgAct = bfdUdCounterCtrl->getFeatures()->getTriggerActions();
...
void CConfigurationDlg::OnBnClickedCancel()
{
   ...
   if (!windowText.Compare(_T("Counter_BufferedUpDownCounter")))
   {
...
void CConfigurationDlg::OnCbnSelchangeCbxTrgsrc()
{
   ...
   wchar_t w_des[MAX_DEVICE_DESC_LEN];
   DeviceInformation devInfo( TCHAR_TO_WCHAR((LPCTSTR)des,  w_des) );
   bfdUdCounterCtrl->setSelectedDevice(devInfo);

   if (bfdUdCounterCtrl->getTrigger()){
...
void CConfigurationDlg::ComponentEnabled(bool flag)
{
   if (flag){
      ...
      ((CWnd*)GetDlgItem(IDC_CBX_SampleClkSrc))->EnableWindow(FALSE);
      ((CWnd*)GetDlgItem(IDC_CBX_CountingType))->EnableWindow(FALSE);
      ...
      ((CWnd*)GetDlgItem(IDC_CBX_SampleClkSrc))->EnableWindow(TRUE);
      ((CWnd*)GetDlgItem(IDC_CBX_CountingType))->EnableWindow(TRUE);
