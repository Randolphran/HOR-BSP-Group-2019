...
DevConfParam CConfigurationDlg::GetParameter()
{
   ...
   param.waveformOutFreq = m_waveformOutFreq;
...
void CConfigurationDlg::DoDataExchange(CDataExchange* pDX)
{
   ...
   DDX_Text(pDX, IDC_WaveformOutFreq, m_waveformOutFreq);
}
...
void CConfigurationDlg::OnBnClickedOk() 
{
   ...
   if (m_waveformOutFreq <= 0 || m_pointsCount <= 0)
   {
      AfxMessageBox(_T("Points for each waveform and waveform out frequency should be greater than 0."));
      return;
   }
...
void CConfigurationDlg::OnBnClickedCancel()
{
   ...
   if (!windowText.Compare(_T("Streaming AO")))
   {
