...
DevConfParam CConfigurationDlg::GetParameter()
{
   ...
   param.moduleIndex = (int)m_comboBox_moduleIndex.GetItemData(m_comboBox_moduleIndex.GetCurSel());
...
void CConfigurationDlg::DoDataExchange(CDataExchange* pDX)
{
   ...
   DDX_Control(pDX, IDC_CounterChannel, m_comboBox_counterChan);
	DDX_Control(pDX, IDC_ModuleIndex, m_comboBox_moduleIndex);
}
...
BEGIN_MESSAGE_MAP(CConfigurationDlg, CDialog)
   ...
   ON_CBN_SELCHANGE(IDC_ModuleIndex, OnCbnSelchangeComboModule)
	ON_WM_CLOSE()
...
BOOL CConfigurationDlg::OnInitDialog()
{
   ...
   Array<DeviceTreeNode>* sptedDevices = timerPulseCtrl->getSupportedDevices();
...
void CConfigurationDlg::OnCbnSelchangeComboDevice()
{
	...
	m_comboBox_moduleIndex.ResetContent();
   ...
   TimerPulseCtrl* timerPulseCtrl = TimerPulseCtrl::Create();
	...
	if (sptedDevices != NULL)
	{
		for (int i = 0; i < sptedDevices->getCount(); ++i)
		{
			DeviceTreeNode devNote = sptedDevices->getItem(i);
			if (devNote.DeviceNumber == (int)m_comboBox_Device.GetItemData(m_comboBox_Device.GetCurSel()))
			{
				int moduleCount = 0;
				int index = 0;
				for (int j = 0; j < 8; ++j)
				{
					...
					if (index != -1)
					{
						...
						m_comboBox_moduleIndex.AddString(str);
						m_comboBox_moduleIndex.SetItemData(moduleCount++,index);
					}else{
						break;
					}
				}
				m_comboBox_moduleIndex.SetCurSel(0);
				break;
			}
		}
	}
   ...
   ErrorCode errorCode = timerPulseCtrl->setSelectedDevice(devInfo);
...
void CConfigurationDlg::OnCbnSelchangeComboModule()
{
   // Clear the context of combo box for parameters
   m_comboBox_counterChan.ResetContent();

   int moduleIndex = (int)m_comboBox_moduleIndex.GetItemData(m_comboBox_moduleIndex.GetCurSel());
   // TODO: Add your control notification handler code here
   TimerPulseCtrl* timerPulseCtrl = TimerPulseCtrl::Create();
   ...
   m_comboBox_Device.GetLBText(m_comboBox_Device.GetCurSel(),des);
   wchar_t w_des[MAX_DEVICE_DESC_LEN];
   DeviceInformation devInfo( TCHAR_TO_WCHAR((LPCTSTR)des,  w_des), ModeWriteWithReset, moduleIndex );
   ErrorCode errorCode = timerPulseCtrl->setSelectedDevice(devInfo);
   GetDlgItem(IDOK)->EnableWindow(TRUE);
   if (errorCode != 0){
      CString str;
...
void CConfigurationDlg::OnBnClickedCancel()
{
   ...
   if (!windowText.Compare(_T("Counter_PulseOutputwithTimerInterrupt")))
   {
