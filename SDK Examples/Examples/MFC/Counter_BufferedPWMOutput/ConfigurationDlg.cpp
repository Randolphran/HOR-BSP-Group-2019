...
DevConfParam CConfigurationDlg::GetParameter()
{
   ...
   param.intervelCount = m_intervelCount;
...
void CConfigurationDlg::DoDataExchange(CDataExchange* pDX)
{
   ...
   DDX_Text(pDX, IDC_EDIT_IntervelCount, m_intervelCount);
...
BOOL CConfigurationDlg::OnInitDialog()
{
   ...
   Array<DeviceTreeNode>* sptedDevices = bfdPwModulatorCtrl->getSupportedDevices();
...
void CConfigurationDlg::OnCbnSelchangeComboDevice()
{
   ...
   ErrorCode errorCode = bfdPwModulatorCtrl->setSelectedDevice(devInfo);
 ...
 Select other device please!"),errorCode,des);
   ...
   Array<SignalDrop>* trgSrc = bfdPwModulatorCtrl->getFeatures()->getTriggerSources();
   ...
   Array<TriggerAction>*trgAct = bfdPwModulatorCtrl->getFeatures()->getTriggerActions();
...
void CConfigurationDlg::OnBnClickedCancel()
{
   ...
   if (!windowText.Compare(_T("Counter_BufferedPWMOutput")))
   {
...
void CConfigurationDlg::OnCbnSelchangeCbxTrgsrc()
{
   ...
   wchar_t w_des[MAX_DEVICE_DESC_LEN];
   DeviceInformation devInfo( TCHAR_TO_WCHAR((LPCTSTR)des,  w_des) );
   bfdPwModulatorCtrl->setSelectedDevice(devInfo);

   if (bfdPwModulatorCtrl->getTrigger()){
...
void CConfigurationDlg::ComponentEnabled(bool flag)
{
   if (flag){
      ...
      ((CWnd*)GetDlgItem(IDC_EDIT_IntervelCount))->EnableWindow(FALSE);
      ...
      ((CWnd*)GetDlgItem(IDC_EDIT_IntervelCount))->EnableWindow(TRUE);
