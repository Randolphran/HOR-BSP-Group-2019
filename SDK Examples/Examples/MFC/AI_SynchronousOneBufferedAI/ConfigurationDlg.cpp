...
void CConfigurationDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Text(pDX, IDC_EDIT_RATEPERCHAN, m_clockRatePerChan);
}
...
BEGIN_MESSAGE_MAP(CConfigurationDlg, CDialog)
   //{{AFX_MSG_MAP(CConfigurationDlg)
	...
	ON_EN_CHANGE(IDC_EDIT_SECTIONLENGTH, OnEnChangeEditDatacountperchan)
	ON_EN_CHANGE(IDC_EDIT_RATEPERCHAN, OnEnChangeEditClockrateperchan)
	ON_CBN_SELCHANGE(IDC_COMBO_CHANCOUNT, OnCbnSelchangeComboChancount)
	ON_WM_CLOSE()
   //}}AFX_MSG_MAP
...
BOOL CConfigurationDlg::OnInitDialog()
{
   ...
   wfAiCtrl ->Dispose();
   OnCbnSelchangeComboChancount();
   OnEnChangeEditDatacountperchan();
...
void CConfigurationDlg::OnCbnSelchangeComboDevice()
{
 ...
 Select other device please!"),errorCode,des);
   ...
   {
      errorCode = AdxGetValueRangeInformation((ValueRanges->getItem(i)), sizeof(vrgDescription), vrgDescription, &ranges, &unit);
...
void CConfigurationDlg::OnBnClickedCancel()
{
   ...
   if (!windowText.Compare(_T("Synchronous One Buffered AI")))
   {
...
void CConfigurationDlg::OnEnChangeEditDatacountperchan()
{
	...
	UpdateData(TRUE);
	//int channelCount = m_comboBox_ChannelCount.GetCurSel() + 1;
	//m_totalTime = m_dataCountPerChan * 1000.0 / m_clockRatePerChan;
	//UpdateData(FALSE);
}
...
void CConfigurationDlg::OnCbnSelchangeComboChancount()
{
	// TODO: Add your control notification handler code here
	int channelCount = m_comboBox_ChannelCount.GetCurSel() + 1;
	UpdateData(FALSE);
