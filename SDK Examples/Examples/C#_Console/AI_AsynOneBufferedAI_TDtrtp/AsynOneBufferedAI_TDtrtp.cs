...
*
* Windows Example:
*    AsynOneBufferedAI_TDtrtp.cs
*
* Example Category:
*    AI
*
* Description:
*    This example demonstrates how to use Asynchronous One Buffered AI with Delay to Start and delay to stop Trigger function together.
*
   ...
   class AsynchronousOneBufferedAI
   {
      static void Main(string[] args)
      {
         ...
         //-----------------------------------------------------------------------------------
			...
			string profilePath = "../../../profile/PCIE-1810.xml";
			int startChannel = 0;
			int channelCount = 2;
      ...
      static void waveformAiCtrl_Stopped(object sender, BfdAiEventArgs e)
      {
         ...
         waveformAiCtrl.GetData(getDataCount, dataBuffer, 0, out returnedCount);
         Trigger trigger = waveformAiCtrl.Trigger1;
         int delayCount = 0;
         if (trigger != null)
         {
            delayCount = waveformAiCtrl.Trigger1.DelayCount; 
            int triggerPointIndex = returnedCount/chanCount - delayCount;
				Console.WriteLine(" The data count each channel:{0},  trigger point each channel: {1}\n\n", returnedCount / chanCount, triggerPointIndex);
         }
         Console.WriteLine(" The first sample each channel are:\n");
         for (int i = 0; i < chanCount; ++i)
         {
             Console.WriteLine(" Channel {0}: {1,13:f8}", (i % chanCount + startChan) % channelCountMax,dataBuffer[i]);
         }
      }
