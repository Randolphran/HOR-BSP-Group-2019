...
Public Class AsynOneBufferedAI_TDtrForm
    Inherits Form
#Region "fields"
...
#End Region
    ...
    Private Sub AsynchronousOneBufferedAIForm_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
		...
		If Not WaveformAiCtrl1.Initialized Then
			MessageBox.Show("Please select a device in control property!", "AsynOneBufferedAI_TDtr")
			Me.Close()
			Return
		End If
		...
		Me.Text = "Asynchronous One Buffered AI with Trigger Delay to Start - Run(" + WaveformAiCtrl1.SelectedDevice.Description + ")"
    ...
    Private Sub HandleError(ByVal err As ErrorCode)
        If err <> ErrorCode.Success Then
            MessageBox.Show("Sorry ! Some errors happened, the error code is: " & err.ToString(), "AsynOneBufferedAI_TDtr")
        End If
    End Sub
