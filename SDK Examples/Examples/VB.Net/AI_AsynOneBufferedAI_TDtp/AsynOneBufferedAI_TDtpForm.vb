...
Public Class AsynOneBufferedAI_TDtpForm
    Inherits Form
#Region "fields"
...
#End Region
    ...
    Private Sub AsynchronousOneBufferedAIForm_Load(ByVal sender As Object, ByVal e As EventArgs) Handles MyBase.Load
		...
		If Not WaveformAiCtrl1.Initialized Then
			MessageBox.Show("Please select a device in control property!", "AsynOneBufferedAI_TDtp")
			Me.Close()
			Return
		End If
		...
		Me.Text = "Asynchronous One Buffered AI with Trigger Delay to Stop - Run(" + WaveformAiCtrl1.SelectedDevice.Description + ")"
      ...
      label_triggerPoint.Hide()
		...
		If WaveformAiCtrl1.Features.TriggerSupported Then
			...
			If Not WaveformAiCtrl1.Trigger.Source = SignalDrop.SignalNone Then
   ...
   End Sub
    ...
    Private Sub trackBar_shift_Scroll(ByVal sender As Object, ByVal e As EventArgs) Handles trackBar_shift.Scroll
		...
		If WaveformAiCtrl1.Features.TriggerSupported And Not (WaveformAiCtrl1.Trigger.Source = SignalDrop.SignalNone) Then
			SetTriggerPointFlagLoacation(WaveformAiCtrl1.Trigger.DelayCount)
		End If
	...
	Private Sub waveformAiCtrl1_Stopped(ByVal sender As Object, ByVal args As BfdAiEventArgs) Handles WaveformAiCtrl1.Stopped
		...
		Try
			...
			m_simpleGraph.Chart(m_dataScaled, chanCount, realCount / chanCount, 1 / WaveformAiCtrl1.Conversion.ClockRate)
    ...
    Private Sub UpdateUIMethod()
		...
		If WaveformAiCtrl1.Features.TriggerSupported Then
			...
			Dim delayCount As Integer = WaveformAiCtrl1.Trigger.DelayCount
			Dim rate As [Double] = WaveformAiCtrl1.Conversion.ClockRate
			If rate > 1000 Then
				rate /= 100
			End If
			...
			Dim relativeWidth As Integer = GetArrowXCordLocation(dataReadyCount, WaveformAiCtrl1.Trigger.DelayCount, pictureBox.Size.Width)
			If relativeWidth = 0 Then
				triggerPointFlag.Hide()
				label_triggerPoint.Hide()
			Else
