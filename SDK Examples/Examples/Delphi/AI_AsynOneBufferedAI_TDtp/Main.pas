...
type
  TMainForm = class(TForm)
    ...
    triggerPointFlag: TImage;
    label_triggerPoint: TLabel;
  ...
  private
    ...
    dataReadyCount : Integer;
    procedure HandleError(err : ErrorCode);
    procedure InitializeGraph;
    procedure InitializeGraph2(sampleCountPerChan : integer);
    function GetArrowXCordLocation(dataCount, delayCount, pixelCount : integer) : integer;
    procedure SetTriggerPointFlagLoacation(delayCount : integer); 
  public
    { Public declarations }
  end;
...
begin
   ...
   begin
      MessageBox(self.WindowHandle, 'Please select a device with DAQNavi wizard!', 'AsynOneBufferedAI_TDtp', MB_OK or MB_ICONERROR);
   ...
   self.Caption := 'Asynchronous One Buffered AI with Trigger Delay to Stop(' + devDesc + ')';
...
procedure TMainForm.InitializeGraph2(sampleCountPerChan : integer);
var
   vrgUnit : ValueUnit;
   vrgRange : MathInterval;
   ustr : string;
...
begin
   //time track bar
   shiftTrackBar.Max := Trunc(1000 * sampleCountPerChan / BufferedAiCtrl1.ConvertClock.Rate);
   shiftTrackBar.Min      := 0;
   shiftTrackBar.Position := 0;
   shiftTimeEditor.Text   := '0';
   shiftTimeLabel.Caption := 'ms';
   ...
   begin
      ...
      begin
         ...
         begin
            ...
            MessageBox(self.WindowHandle, PChar(errMessage), 'AsynOneBufferedAI_TDtp', MB_OK or MB_ICONERROR);
            exit;
         end
         else if BufferedAiCtrl1.Trigger.DelayCount > BufferedAiCtrl1.ScanChannel.Samples then
         begin
            MessageBox(self.WindowHandle, 'Trigger DelayCount should be smaller than scanChannel Samples', 'AsynOneBufferedAI_TDtp', MB_OK or MB_ICONERROR);
            exit;
         end;
      end;
   end;
...
var
   err : ErrorCode;
   chanCount, totalDataCountPerChan, delayCount, relativeWidth : integer;
   rate, triggerTime, timeFactor : double;
   timeUnit : string;
begin
   err := Success;
   ...
   InitializeGraph2(dataReadyCount div chanCount);

   //draw the data to the PaintBox
   dataScaledArray := dataScaled;
   m_graph.Chart(dataScaledArray, BufferedAiCtrl1.ScanChannel.ChannelCount, dataReadyCount div BufferedAiCtrl1.ScanChannel.ChannelCount, 1.0 / BufferedAiCtrl1.ConvertClock.Rate);

   BtnStart.Enabled := true;

   if BufferedAiCtrl1.Features.TriggerSupported then
   begin
      totalDataCountPerChan := dataReadyCount div BufferedAiCtrl1.ScanChannel.ChannelCount;
      ...
      label_triggerPoint.Caption := format('%.2f', [triggerTime]) + ' ' + timeUnit + ' Triggered.';
      
      relativeWidth := GetArrowXCordLocation(dataReadyCount div BufferedAiCtrl1.ScanChannel.ChannelCount, BufferedAiCtrl1.Trigger.delayCount, DataView.Width);
...
function TMainForm.GetArrowXCordLocation(dataCount, delayCount, pixelCount : integer) : integer;
var
   relativePixelWidth, screenCount, shiftCount, shift, divide : integer;
   rate : double;
begin
   relativePixelWidth := 0;
   rate := BufferedAiCtrl1.ConvertClock.Rate;
   shift := shiftTrackBar.Position;
   divide := divTrackBar.Position;

   if rate >= 10000 then
      rate := rate / 1000;

   screenCount := Trunc(divide * 10 * rate / 1000);
   shiftCount := Trunc(shift * rate / 1000);

   if (dataCount - delayCount - shiftCount > 0) and (dataCount - delayCount - shiftCount < screenCount) then
      relativePixelWidth := pixelCount * (dataCount - delayCount - shiftCount) div screenCount
   else
      relativePixelWidth := 0;
...
procedure TMainForm.SetTriggerPointFlagLoacation(delayCount : integer);
var
   dataCount, relativeWidth : integer;
begin
   dataCount := dataReadyCount div BufferedAiCtrl1.ScanChannel.ChannelCount;
   relativeWidth := GetArrowXCordLocation(dataCount, delayCount, DataView.Width);
   ...
   if triggerPointFlag.Left < 52 then
   begin
      label_triggerPoint.Visible := false;
      triggerPointFlag.Visible := false;
   end;
