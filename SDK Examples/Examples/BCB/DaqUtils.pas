...
type
   ...
   private
      ...
      m_xPosition      : Integer;
      m_recordEndFlag  : Boolean;

      m_drawRecordEnd  : Boolean;
      m_recordEndPointx: Integer;
   ...
   public
      ...
      procedure Clear;
      procedure Chart(const data: array of Double; plotCount, dataPerPlot: Integer; xIncBySec: Double; flag: Boolean);overload;
      procedure Chart(const data: array of Double; plotCount, dataPerPlot: Integer; xIncBySec: Double);overload;
...
begin
   ...
   if m_dataCachedPerPlot - m_shiftCount - m_mapIndexPerPlot > m_pointPerScreen then
         count := m_pointPerScreen
   else  count := m_dataCachedPerPlot - m_shiftCount - m_mapIndexPerPlot;
   ...
   if (m_drawRecordEnd = true) then
   begin
       m_recordEndPointX := Round(m_xPosition * m_XCordDivRate);
   end;
end;
...
begin
   ...
   if (m_drawRecordEnd = true) then
   begin
      ptS.X := m_recordEndPointX;
      ptS.Y := 0;
      ptE.X := ptS.X;
      ptE.Y := m_hostCtrl.Height - 1;
      canvas.Pen.Color := $E1E4FA;
      canvas.Polyline([ptS, ptE]);
   end;
   ...
   begin
      count := Min(m_dataCachedPerPlot - m_shiftCount - m_mapIndexPerPlot, m_pointPerScreen);
      if count > 0 then
...
begin
  Chart(data, plotCount, dataPerPlot, xIncBySec, false);
end;


procedure TSimpleGraph.Chart(const data: array of Double; plotCount, dataPerPlot: Integer; xIncBySec: Double; flag: Boolean);
begin
...
begin
   ...
   begin
      if m_overOneScreenMode = EndScreen then
       m_mapIndexPerPlot := dataPerPlot - m_pointPerScreen - 1;
    ...
    xPos := m_dataCachedPerPlot - m_mapIndexPerPlot - 1;
    if (m_recordEndFlag = true) then
    begin
        m_xPosition := xPos;
        m_drawRecordEnd := true;
    end else begin
        m_xPosition := m_xPosition - offset;
        m_drawRecordEnd := false;
        if (m_xPosition > 0) and ( m_xPosition < m_pointPerScreen) then
          m_drawRecordEnd := true;
    end;
end;
