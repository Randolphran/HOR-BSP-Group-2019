...
void TMainForm::Initialization() {
        ...
        AnsiString caption = "Streaming AO - Run("  + deviceName + ")";
...
void TMainForm::ConfigureDevice() {
        ...
        CheckError(ret);
        ret = scanChannel->setIntervalCount(configure.pointCountPerWave);
        CheckError(ret);
        ret = bfdAoCtrl->getConvertClock()->setRate(configure.clockRate * configure.pointCountPerWave);
        CheckError(ret);
...
//----------------------------------------------------------------------------
void BDAQCALL TMainForm::OnStoppedEvent(void *sender, BfdAoEventArgs * args, void * userParam)
{
        TMainForm * uParam = (TMainForm *)userParam;
        MessageBox(uParam->WindowHandle, "EvtBufferedAOStopped!", "StreamingAO", MB_ICONERROR);
}
//----------------------------------------------------------------------------
 void BDAQCALL TMainForm::OnUnderrunEvent(void *sender, BfdAoEventArgs * args, void * userParam)
{
        TMainForm * uParam = (TMainForm *)userParam;
        MessageBox(uParam->WindowHandle, "EvtBufferedAOUnderrun!", "StreamingAO", MB_ICONERROR);
}
//-----------------------------------------------------------------------------
...
void __fastcall TMainForm::BtnStartClick(TObject *Sender)
{
        ...
        if (BtnStart->Caption == "Start") {
                ...
                this->txtboxHiLevelA->Enabled = false;
                this->txtboxHiLevelB->Enabled = false;
                this->txtboxLoLevelA->Enabled = false;
                this->txtboxLoLevelB->Enabled = false;

                for (int i = 0; i < 6; i++) {
                        m_waveButton[i]->Enabled = false;
                }
                ...
                ret = bfdAoCtrl->Stop(0);
                CheckError(ret);
                
                this->txtboxHiLevelA->Enabled = true;
                this->txtboxHiLevelB->Enabled = true;
                this->txtboxLoLevelA->Enabled = true;
                this->txtboxLoLevelB->Enabled = true;

                for (int i = 0; i < 6; i++) {
                        m_waveButton[i]->Enabled = true;
                }
