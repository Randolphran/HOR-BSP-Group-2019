...
void TMainForm::Initialization() {
        ...
        AnsiString caption = "Asynchronous One Buffered AI with Trigger Delay to Stop - Run("  + deviceName + ")";
...
void TMainForm::InitializeGraph(int samplesCountPerChan) {
        ...
        double timeInterval = 100.0 * DataView->Width / convertRate;
        double shiftMax = 1000.0 * samplesCountPerChan / wfAiCtrl->getConversion()->getClockRate();
        while (convertRate >= 10 * 1000)
        {
         timeInterval *= 1000;
         shiftMax *= 1000;
