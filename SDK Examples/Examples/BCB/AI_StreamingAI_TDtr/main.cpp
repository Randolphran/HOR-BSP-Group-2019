...
void TMainForm::Initialization() {
        ...
        AnsiString caption = "Streaming AI with Trigger Delay to Start - Run("  + deviceName + ")";
        ...
        dataScaled = new double[channelCount * sectionLength];
...
void BDAQCALL TMainForm::OnDataReadyEvent(void *sender, BfdAiEventArgs * args, void * userParam)
{
        ...
        //get data
        ret = wfAiCtrl->GetData(args->Count, uParam->dataScaled, 0, NULL, NULL, NULL, NULL);
        ...
        int dataPerPlot = uParam->configure.sectionLength;
        uParam->m_graph->Chart(uParam->dataScaled, args->Count, uParam->configure.channelCount, dataPerPlot, 1.0 / wfAiCtrl->getConversion()->getClockRate());
}
//-----------------------------------------------------------------------------
