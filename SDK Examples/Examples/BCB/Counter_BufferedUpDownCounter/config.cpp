...
void TConfigForm::Initialization()
{
        ...
        Array<DeviceTreeNode> *supportedDevice = m_bfdUdCounterCtrl->getSupportedDevices();
...
void __fastcall TConfigForm::cmbDeviceChange(TObject *Sender)
{
        ...
        ret = m_bfdUdCounterCtrl->setSelectedDevice(devInfo);
        if (ret != Success) {
                int err = ret;
        ...
        channelCountMax = m_bfdUdCounterCtrl->getFeatures()->getChannelCountMax();
	...
	wchar_t enumStr[MAX_SIG_DROP_DESC_LEN] = {0};
	for (int i = 0; i < cntTypeCount; ++i) {
		AdxEnumToString(L"CountingType", countingType->getItem(i), MAX_SIG_DROP_DESC_LEN, enumStr);
                cbxCountingType->AddItem(enumStr, this);
	}
        cbxCountingType->ItemIndex = 0;
        AnsiString source = cbxTrgSource->Text;
        if (source == "None") {
...
void __fastcall TConfigForm::btnOkClick(TObject *Sender)
{
        ...
        if(radProfile->Checked){
                ...
                Array<CountingType> * countingType = m_bfdUdCounterCtrl->getFeatures()->getUdCountingTypes();
                configure.cntType = countingType->getItem(cbxCountingType->ItemIndex);

                CounterClockSourceIndexer* indexer = m_bfdUdCounterCtrl->getFeatures()->getUdSampleClockSources();
                configure.clkSource = indexer->getItem(0)->getItem(cbxSampleClkSrc->ItemIndex);
...
void  TConfigForm::RefreshConfigureParameter()
{
        ...
        txtSectionLength->Text = bfdUdCounterCtrl->getRecord()->getSectionLength();
        txtSectionCount->Text = bfdUdCounterCtrl->getRecord()->getSectionCount();

        SignalDrop clkSrc = bfdUdCounterCtrl->getSampleClkSrc();
        ...
        CountingType countingType = bfdUdCounterCtrl->getChannels()->getItem(0).getCountingType();
        Array<CountingType> * type = bfdUdCounterCtrl->getFeatures()->getUdCountingTypes();
        for(int i = 0; i < type->getCount(); i++){
                if(countingType == type->getItem(i)){
                        cbxCountingType->ItemIndex = i;
                        break;
                }
        }

        SignalDrop trgSrc = bfdUdCounterCtrl->getTrigger()->getSource();
        Array<SignalDrop>*  sources = bfdUdCounterCtrl->getFeatures()->getTriggerSources();
        ...
        TriggerAction trgAction = bfdUdCounterCtrl->getTrigger()->getAction();
        Array<TriggerAction>* action = bfdUdCounterCtrl->getFeatures()->getTriggerActions();
        ...
        ActiveSignal trgEdge = bfdUdCounterCtrl->getTrigger()->getEdge();
        int index = -1;
        trgEdge == RisingEdge ? (index = 0) : (index = 1);
        cbxTrgEdge->ItemIndex = index;

        txtTrgLevel->Text = bfdUdCounterCtrl->getTrigger()->getLevel();

        bfdUdCounterCtrl->getChannelMap(channelCountMax, configure.channelEnable);
        for(int i = 0; i < channelCountMax; i++){
