...
void TMainForm::Initialization() {
        ...
        AnsiString caption = "Streaming AI with Retrigger - Run("  + deviceName + ")";
        ...
        channelCount = configure.channelCount;
        dataCount = sectionLength * configure.channelCount;
...
void TMainForm::ConfigureDevice() {
        ...
        CheckError(ret);
        // The new function used to load the profile.
        ret = wfAiCtrl->LoadProfile(configure.profileName);
        CheckError(ret);
        ...
        CheckError(ret);
        ret = conversion->setClockRate(configure.clockRate);
        CheckError(ret);
        Record * record = wfAiCtrl->getRecord();
        ret = record->setSectionLength(configure.sectionLength);
        CheckError(ret);
        ret = record->setSectionCount(0);//0 means streaming mode.
        CheckError(ret);
        ret = record->setCycles(configure.cycles);
        CheckError(ret);

        for (int i = 0; i < wfAiCtrl->getChannels()->getLength(); i++) {
        ...
        if (trigger1 != NULL){
                ...
                trigger1->setDelayCount(configure.delay1Count);
                trigger1->setEdge(configure.trigger1Edge);
                trigger1->setLevel(configure.trigger1Level);
        }

        delete[] configure.profileName;
        configure.profileName = NULL;
}
//---------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
void BDAQCALL TMainForm::OnDataReadyEvent(void *sender, BfdAiEventArgs * args, void * userParam)
{
	...
	double timeStamp = 0;
        ...
        //get data
        ret = wfAiCtrl->GetData(getDataCount, uParam->dataScaled, -1, &returnedCount,&timeStamp, NULL, NULL);
        flagForRecordEnd = (ret == WarningRecordEnd)? true : false;

        //draw the data to the PaintBox
        uParam->m_graph->Chart(uParam->dataScaled, getDataCount,
                               uParam->configure.channelCount, uParam->sectionLength,
                               1.0 / wfAiCtrl->getConversion()->getClockRate(), flagForRecordEnd);
}
//-----------------------------------------------------------------------------

void BDAQCALL TMainForm::OnOverrunEvent(void * sender, BfdAiEventArgs * args, void * userParams)
{
        TMainForm * uParam = (TMainForm *)userParams;
        MessageBox(uParam->WindowHandle, "BufferedAiOverrun!", "StreamingAI", MB_ICONERROR);
}
//-----------------------------------------------------------------------------
 void BDAQCALL TMainForm::OnCacheOverflowEvent(void * sender, BfdAiEventArgs * args, void * userParams)
 {
        TMainForm * uParam = (TMainForm *)userParams;
        if (uParam->isFirstOverRun == true) {
                MessageBox(uParam->WindowHandle, "BufferedAiCacheOverflow!", "StreamingAI", MB_ICONERROR);
                uParam->isFirstOverRun = false;
        }
 }
//-----------------------------------------------------------------------------
