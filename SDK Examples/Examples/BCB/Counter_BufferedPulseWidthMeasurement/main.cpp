        ...
        : TForm(Owner)
{
        ...
        bfdPwMeterCtrl->addDataReadyHandler(OnDataReadyEvent, this);
        bfdPwMeterCtrl->addStoppedHandler(OnStoppedEvent, this);
        dataBuf = NULL;
        chs_dataBuf = NULL;
        channelEnable = NULL;
        chs_Status = NULL;
...
void TMainForm::Initialization() {
        ...
        AnsiString caption = "Buffered Pulse Width Measurement - Run("  + deviceName + ")";
...
void TMainForm::ConfigureDevice() {
        ...
        if(configure.configureMode == Profile_Confiugre){
                ret = bfdPwMeterCtrl->LoadProfile(configure.profilePath);
                CheckError(ret);
                ...
                ret = bfdPwMeterCtrl->setChannelMap(channelCountMax, configure.channelEnable);
                CheckError(ret);

                ret = bfdPwMeterCtrl->setSampleClkSrc(configure.clkSource);
                CheckError(ret);
        ...
        memset(dataBuf, 0, sizeof(PulseWidth) * samplesCount);

        chs_dataBuf = new PulseWidth* [channelCountMax];
        for(int i = 0; i < channelCountMax; i++){
                chs_dataBuf[i] = new PulseWidth[samplesCount];
                memset(chs_dataBuf[i], 0, sizeof(PulseWidth) * samplesCount);
        }
...
void TMainForm::DisplayChannelValue(int32 channel)
{
        ...
        if(haveData | isRunning){
                for(int i = 0; i < listCount; i++){
                        str = "High:" + FormatFloat("0.000000", chs_dataBuf[ch][i].HiPeriod) + " s  Low:" + FormatFloat("0.000000", chs_dataBuf[ch][i].LoPeriod) + " s";
...
{
        int ch = channel;
        memcpy(chs_dataBuf[ch], dataBuf, sizeof(PulseWidth) * samplesCount);
}
