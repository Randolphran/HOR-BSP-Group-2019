...
void TMainForm::Initialization() {
        ...
        AnsiString caption = "Synchronous One Buffered AI - Run("  + deviceName + ")";
        ...
        int sectionCount = 1;
        getDataCount =  channelCount * sectionLength * sectionCount;
        dataScaled = new double[getDataCount];
...
void __fastcall TMainForm::BtnStartClick(TObject *Sender)
{
        ...
        if (ret == Success) {
                //get data
                ret = wfAiCtrl->GetData(getDataCount, dataScaled, -1, NULL, NULL, NULL, NULL);
        }
        ...
        m_graph->Chart(dataScaled, getDataCount, configure.channelCount, configure.sectionLength, 1.0 / wfAiCtrl->getConversion()->getClockRate());
