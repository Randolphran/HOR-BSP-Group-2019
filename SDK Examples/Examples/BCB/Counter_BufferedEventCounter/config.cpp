...
void TConfigForm::Initialization()
{
        ...
        Array<DeviceTreeNode> *supportedDevice = m_bfdEventCounterCtrl->getSupportedDevices();
...
void __fastcall TConfigForm::cmbDeviceChange(TObject *Sender)
{
        ...
        ret = m_bfdEventCounterCtrl->setSelectedDevice(devInfo);
        if (ret != Success) {
                int err = ret;
        ...
        radioBtnClick(this);
        channelCountMax = m_bfdEventCounterCtrl->getFeatures()->getChannelCountMax();
...
{
        BufferedUdCounterCtrl* bfdEventCounterCtrl = BufferedUdCounterCtrl::Create();
        ...
        txtSectionLength->Text = bfdEventCounterCtrl->getRecord()->getSectionLength();
        txtSectionCount->Text = bfdEventCounterCtrl->getRecord()->getSectionCount();

        SignalDrop clkSrc = bfdEventCounterCtrl->getSampleClkSrc();
        ...
        SignalDrop trgSrc = bfdEventCounterCtrl->getTrigger()->getSource();
        Array<SignalDrop>*  sources = bfdEventCounterCtrl->getFeatures()->getTriggerSources();
        ...
        TriggerAction trgAction = bfdEventCounterCtrl->getTrigger()->getAction();
        Array<TriggerAction>* action = bfdEventCounterCtrl->getFeatures()->getTriggerActions();
        ...
        ActiveSignal trgEdge = bfdEventCounterCtrl->getTrigger()->getEdge();
        int index = -1;
        trgEdge == RisingEdge ? (index = 0) : (index = 1);
        cbxTrgEdge->ItemIndex = index;

        txtTrgLevel->Text = bfdEventCounterCtrl->getTrigger()->getLevel();

        bfdEventCounterCtrl->getChannelMap(channelCountMax, configure.channelEnable);
        for(int i = 0; i < channelCountMax; i++){
